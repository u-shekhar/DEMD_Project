...
# Now creating the one hot encoded version for the data frame. Here we will have a column for each genre, 
# representing the genres for every single row of movies.
...
def movie_recommender(title_1,rating_1,title_2,rating_2,title_3,rating_3,title_4,rating_4,title_5,rating_5,movies_df,moviesWithGenres_df) :
    ...
    # To add more movies, we can simply increase the movies in the userInput. 
    # We should write it in capital letters and if a movie starts with a "The", 
    # like "The Matrix" then write it in like this: 'Matrix, The'.
    ...
    # If the movie is not in our original dataset or is mis spelled then it could not be found then, 
    # it won't show up in the inputMovies dataframe.
    ...
    # If a movie we added above, is not here, then it might not be present in the original data frame or it might spelled wrong. 
    ...
    # From the userGenreTable, we can definitely tell which genres he has watched or based on these 5 movies, 
    # we can determine the genres to which these movies fall into.
    # By applying the ratings as weight and aggregating the scores for each genre, 
    # we can identify which would be the movies that the user likes the most.

    # Now we can use the ratings for each movie and multiply it with the genres and 
    # sum up the values for a particular genre to get the weight of the genre. 
    # This way we can get the user profile.

    # Creating the user profile

    # Now multiplying the rating with the genre table and summing up to get the genre weights 
    # for that particular user i.e creating a user profile.

    # For multiplying the userGenreTable and inputMovies rating column, the dimension of them should match like, 
    # if one has n rows and the other should have n columns.
    # Since, we want the final output as the weighted scores of all the genres, we will have to take transpose of it and 
    # multiply by ratings. This way we match the dimension requirement as well as the we get the desired output.
    # It would be like the first genre Adventure field for each movies would be multiplied to corresponding ratings and 
    # summed up to give one value for Adventure genre. That would be the user preference for the genre Adventure. 
    ...
    #  From this we can make sure we never suggest the user the movies which fall into the genres 
    #  which have a score of 0 in the user profile. We can suggest the movies that fall into the genres with maximum scores.
    ...
    # Now we have the prefect list with the user preference scores for all the other movies. 
    # Now, we can sort this in descending order and suggest the top 20 movies to the user.
...
# Routing to the data base
@app.route('/movies_database')
# Function that would be executed when the above URL is hit.
def show_db():
...
def get_req_parameters():
    ...
    responses:
       200:
            description: Recommended Movies are
            content:
              application/json:
    ...
    # Calling the movie recommender function defined above.
    ...
    # Getting the recommendations from the dataframe to variable to send as result.
